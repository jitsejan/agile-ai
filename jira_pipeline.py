...
def load(endpoints: Optional[List[str]] = None) -> None:
    ...
    load_info = pipeline.run(jira().with_resources(*endpoints), write_disposition="append")
...
def load_query_data(queries: List[str]) -> None:
    ...
    load_info = pipeline.run(jira_search().issues(jql_queries=queries), write_disposition="append")
